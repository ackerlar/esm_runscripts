...
class Namelist:
    ...
    def apply_echam_lgfw(config):
        ...
        if "echam" in config["general"]["valid_model_names"]:
            ...
            nml = config["echam"]["namelists"]["namelist.echam"]
            # Get the current dynctl chapter or make a new empty one:
            submodelctl = nml.get("submodelctl", f90nml.namelist.Namelist())
            # Determine which years the user wants to have disturbed:
            if config["general"]["iterative_coupling"] and config["general"]["first_run_in_chunk"]:
                submodelctl["lgfw"] = True 
            else:
                submodelctl["lgfw"] = False
            nml["submodelctl"] = submodelctl
        return config

    @staticmethod
    def apply_jsbach_lgfw(config):
        """
        """
        if "jsbach" in config["general"]["valid_model_names"]:
            # Get the echam namelist:
            nml = config["jsbach"]["namelists"]["namelist.jsbach"]
            # Get the current dynctl chapter or make a new empty one:
            hydrology_ctl = nml.get("hydrology_ctl", f90nml.namelist.Namelist())
            # Determine which years the user wants to have disturbed:
            if config["general"]["iterative_coupling"] and config["general"]["first_run_in_chunk"]:
                hydrology_ctl["lgfw"] = True 
            else:
